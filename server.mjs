...
const 源码目录 = resolve(import.meta.dirname, 'sources');
...
服务.listen(端口, () => {
  console.log(`CFnew 部署器已启动: http://localhost:${端口}`);
});
...
async function 部署(数据) {
  ...
  const 项目名 = 操作模式 === 'update' ? 原始项目名 : 清理项目名(原始项目名 || 生成随机名称('cfnew'));
  ...
  const 命名空间 = await 获取或创建KV(数据.credentials, 数据.accountId, {
    id: 数据.kvId,
    title: 数据.kvTitle || 生成随机名称('cfnew-kv')
  }, 记录);
  if (命名空间.created) {
...
async function 补全部署默认值(数据) {
  ...
  if (输出.autoDomain && !输出.hostname) {
    ...
    if (zones.length) {
      ...
      输出.hostname = `${生成随机名称('cfnew')}.${zones[0].name}`;
    }
  }
  ...
  if (输出.hostname && 输出.zoneId) {
    ...
    if (zone && 输出.hostname === zone.name) {
      输出.hostname = `${生成随机名称('cfnew')}.${zone.name}`;
      输出.zoneName = zone.name;
    }
  }
  return 输出;
...
async function 部署Pages(凭据, 选项, 记录) {
  ...
  const 临时目录 = await mkdtemp(join(tmpdir(), 'cfnew-pages-'));
  try {
    const 代码 = await 读取源代码(选项.sourceMode);
    await writeFile(join(临时目录, '_worker.js'), 代码, 'utf8');
    await writeFile(join(临时目录, 'index.html'), '<!doctype html><meta charset="utf-8"><title>CFnew</title>', 'utf8');
...
async function 同步Pages代码(凭据, 选项, 记录) {
  ...
  const 临时目录 = await mkdtemp(join(tmpdir(), 'cfnew-pages-update-'));
  try {
    const 代码 = await 读取源代码(选项.sourceMode);
    await writeFile(join(临时目录, '_worker.js'), 代码, 'utf8');
...
function 清理项目名(名称) {
  return String(名称 || 生成随机名称('cfnew')).trim().toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '') || 生成随机名称('cfnew');
}
